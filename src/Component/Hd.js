...
export default function Hd() {
    ...
    return (
        <header id="hd" className="container d-flex fixed-top border-bottom justify-content-between">
            <h1><a href="#" className={hd.gnblist}><div id='Logo'
            // onMouseOver={() => { setlogo("hover") }} onMouseLeave={() => { setlogo("") }} className={logoover}
            ></div></a></h1>
            <ul id="gnb" className="d-flex">
                {
                    naviArr.map((el, idx) => {
                        return <li key={`list${idx}`} className={`list${idx}`}> <a href={el.href} target={el.target}>{el.nm}</a></li>
                    })
                }
